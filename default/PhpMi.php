...
//Conecting to database
        ...
        if($db == null){
                ...
                if($m->mFiles != null && sizeof($m->mFiles) <= 5) {
                        // Save file to folder
                        foreach ($m->mFiles as $key => $file) {
                                $dir = 'attachments';
                                //if (!file_exists($dir)) {
                ...
                $machineGeneratedFilename = md5( $filename)."_".(rand()%100).".". $fileExt;
                
							   $path = "gs://turnkey-citadel-195504.appspot.com/".'/'.$machineGeneratedFilename;
                                file_put_contents($path, $file['content']);
